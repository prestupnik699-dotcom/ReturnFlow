...
export function DeliveryBatchSheet({ visible, onClose }: Props) {
  const theme = useTheme();
  const { t } = useTranslation();
  const [permission, requestPermission] = useCameraPermissions();
  const { data: suppliers } = useSuppliers(false, 'name');
  const batchMutation = useCreateDeliveriesBatch();
  ...
  const handleBarcodeScanned = async ({ data }: { data: string }) => {
    ...
    setToastMessage(t('deliveries.batch.scanAdded', { title }));
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Text style={styles.title}>{t('deliveries.batch.title')}</Text>
          <Text style={styles.subtitle}>{t('deliveries.batch.subtitle')}</Text>
          ...
          <View style={styles.field}>
            <Text style={styles.label}>{t('deliveries.batch.addManualLabel')}</Text>
          ...
          {lines.length > 0 ? (
            ...
            <Text style={styles.emptyHint}>{t('deliveries.batch.empty')}</Text>
          )}
          ...
          <View style={styles.actions}>
            ...
            />
            <Button
              label={t('deliveries.batch.saveAll', { count: lines.length })}
